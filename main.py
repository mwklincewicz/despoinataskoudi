...
# Comparison of Classifier Performance
   ...
   stds = {
       metric: [np.std(results[m][key]) for m in models_list]
       for metric, key in metrics.items()
   }
...
#####GLOBAL FEATURE IMPORTANCE
   ...
   def collapse_onehot_mean(df_imp):
       df_imp = df_imp.copy()
       df_imp["base_feature"] = (
           df_imp["feature"]
   ...
   def importance_across_folds(results_dict, model_name, importance_fn):
       fold_importances = []
       ...
       for grid in results_dict[model_name]["estimator"]:
           ...
           collapsed = collapse_onehot_mean(df_imp)
   ...
   def plot_feature_importances_subplots(importance_dict, top_k=10):
       ...
       fig, axes = plt.subplots(1, n_models, figsize=(16, 10), sharex=False)
...
#  RUN IMPORTANCE ANALYSIS
   lr_fold_importances = importance_across_folds(results, "Logistic Regression", lr_importance)
   rf_fold_importances = importance_across_folds(results, "Random Forest", rf_importance)
   xgb_fold_importances = importance_across_folds(results, "XGBoost", xgb_importance)
